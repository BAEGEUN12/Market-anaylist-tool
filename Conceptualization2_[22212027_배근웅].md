...
### 4.3 Administer-UI
아래 화면은 관리자 아이디로 로그인 했을때 나타나나는 UI이다. 시장의 트렌드에 맞춰서 여러 기술적 지표를 관리하고 조작할수있다. 
<img width="1121" height="762" alt="image" src="https://github.com/user-attachments/assets/3bb8740e-de8f-4964-a917-b4bd24f68104" />
알고리즘 가중치 제어 (좌측 상단): EvaluationValueManager 클래스의 기능을 구현한 곳으로, 시장의 변동성에 따라 AI가 기술적 지표나 뉴스 데이터를 얼마나 중요하게 다룰지 슬라이더로 즉시 조정할 수 있다.

시스템 요약 (중앙 상단): 현재 접속자 수, API 성공률, 지연 시간 등 서버의 건강 상태를 실시간으로 모니터링한다.

1.UI 컴포넌트 제어 (우측 상단): LayoutAdjustmentManager 클래스의 기능으로, 일반 사용자의 메인 화면에 어떤 위젯을 노출할지 관리자가 스위치 하나로 결정합니다.

2.사용자 관리 (하단 좌측): LoginVerificationManager와 연결되어 현재 누가 시스템에 접속 중인지, 권한은 무엇인지 확인하고 제어할 수 있는 목록이다.

3.트래픽 모니터링 (하단 우측): 서버에 가해지는 부하를 시각화하여, 특정 시간대(예: 장 개시 직후)의 급격한 트래픽 증가에 대비할 수 있게 한다.

이 UI를 통해 시스템의 보안, 성능, 분석 품질을 한 화면에서 효율적으로 관리할 수 있다.

##  5.Glossary

프로젝트 내에서 공통으로 사용되는 주요 용어와 기술적 정의는 다음과 같다.

| Terms | Description |
| :--- | :--- |
| **User (사용자)** | 시스템에 가입하여 주식 분석 정보를 조회하는 일반 권한을 가진 액터(Actor)이다. |
| **Administrator (관리자)** | 시스템의 알고리즘 가중치를 조정하거나 레이아웃을 편집하고, 사용자를 관리하는 상위 권한 액터이다. |
| **Attribute (속성)** | Java와 같은 객체지향 언어에서 클래스 내부에 정의된 데이터 필드(ID, Password, 학과 등)를 의미한다. |
| **Operation (연산)** | 클래스 내부에서 수행되는 기능으로, 프로그래밍 언어의 메서드(Method)에 해당한다. |
| **AnalysisManager** | 주식 데이터 수집과 AI 분석 요청을 총괄하며 전체적인 분석 프로세스를 제어하는 핵심 컨트롤 클래스이다. |
| **Gemini API** | 구글의 대규모 언어 모델 API로, 수집된 주식 데이터를 바탕으로 전문적인 투자 분석 결과와 코멘트를 생성하는 역할을 한다. |
| **Stock API** | 외부 금융 서버로부터 실시간 주가 정보(현재가, 등락률 등)를 가져오기 위한 인터페이스이다. |
| **Evaluation Value (평가 가치)** | AI 분석 시 활용되는 기술적 지표(RSI, MACD 등)의 가중치를 의미하며, 관리자에 의해 실시간으로 조정될 수 있다. |
| **Include (포함 관계)** | 하나의 유스케이스나 클래스 동작 시 반드시 선행되거나 포함되어야 하는 기능 간의 관계(예: 분석 시 로그인 검증 필수)를 의미한다. |
| **Overlap Check (중복 확인)** | 회원가입 시 입력한 ID가 데이터베이스(DB) 내에 이미 존재하는지 확인하여 데이터의 유일성을 보장하는 기능이다. |
| **Layout Adjustment** | 관리자가 대시보드에 노출되는 위젯(차트, 뉴스, 신호 등)의 배치나 활성화 여부를 직접 설정하는 기능이다. |
| **GUI (그래픽 사용자 인터페이스)** | 사용자가 텍스트 대신 아이콘, 버튼, 차트 등 그래픽 요소를 통해 시스템과 상호작용할 수 있게 해주는 화면이다. |

## 6. Reference (참고 문헌 및 자료)

본 프로젝트의 설계 및 구현을 위해 참고한 주요 자료는 다음과 같다.

### 6.1 기술 문서 및 API 가이드
* **Google Gemini API Documentation**: AI 분석 리포트 생성 및 프롬프트 엔지니어링 설계를 위해 참고함
* **Financial Data Provider API Specs**: 실시간 주가 데이터 요청 및 JSON 파싱 구조 설계를 위해 참고함
* **Oracle Java SE Documentation**: 시스템의 핵심 로직인 핸들러(Handler) 및 매니저(Manager) 클래스 구현 방법론 참고

### 6.2 설계 방법론 및 관련 이론
* **UML Distilled (Martin Fowler)**: 유스케이스 다이어그램과 클래스 다이어그램 간의 정적/동적 모델링 설계 규칙 준수
* **객체지향 분석 설계(OOAD)**: 상속(Inheritance) 및 포함(Include) 관계를 활용한 시스템 아키텍처 구조화
* **UI/UX Design Guidelines**: 금융 시스템의 가독성을 높이기 위한 다크 테마 및 데이터 시각화 레이아웃 원칙 참고

### 6.3 프로젝트 산출물
* **Stock Analysis System Use Case Specification**: 각 액터(User, Administrator)별 요구사항 정의서
* **System Class Diagram v1.0**: `AnalysisManager`를 중심으로 한 시스템 전체 클래스 구조 설계도
* **Admin/User UI Prototype**: 주식 분석 플랫폼 및 관리자 제어 대시보드 인터페이스 설계안
