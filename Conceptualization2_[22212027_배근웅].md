...
UI 컴포넌트 제어 (우측 상단): LayoutAdjustmentManager 클래스의 기능으로, 일반 사용자의 메인 화면에 어떤 위젯을 노출할지 관리자가 스위치 하나로 결정합니다.

사용자 관리 (하단 좌측): LoginVerificationManager와 연결되어 현재 누가 시스템에 접속 중인지, 권한은 무엇인지 확인하고 제어할 수 있는 목록이다.

트래픽 모니터링 (하단 우측): 서버에 가해지는 부하를 시각화하여, 특정 시간대(예: 장 개시 직후)의 급격한 트래픽 증가에 대비할 수 있게 한다.
