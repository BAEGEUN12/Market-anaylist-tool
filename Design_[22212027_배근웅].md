...
객체명 (Alias)	상태 (State)	상세 설명
PortalMain (UI)	ReportView	분석 요청을 수신하고 AI가 생성한 최종 리포트 데이터를 사용자 화면에 렌더링하는 인터페이스입니다.
AnalysisEngine (Engine)	DataProcessor	실시간 주식 데이터 수집, 데이터 전처리(Marshalling), 결과물 객체화 등 파이프라인의 핵심 제어 객체입니다.
StockAPI (StockAPI)	MarketData	실시간 시장 데이터를 조회하여 원본 데이터를 제공하는 외부 데이터 인터페이스입니다.
GeminiAPI (GeminiAPI)	AIAnalyzer	전처리된 데이터를 기반으로 심층 분석 리포트를 생성하는 인공지능 분석 서비스입니다.
### 시퀀스 흐름 분석 (Flow Description)
[기본 흐름] 분석 리포트 생성 요청
UI ➔ Engine: 사용자의 종목 분석 요청을 AnalysisEngine으로 전달합니다.

시장 데이터 수집: Engine은 StockAPI를 호출하여 현재가의 Raw Data를 획득합니다. 이때 Engine은 API 응답을 대기하는 동안 비활성화 상태가 되어 리소스를 효율적으로 관리합니다.

AI 분석 전처리: 수집된 시장 데이터를 Gemini API가 이해할 수 있는 최적의 프롬프트 형태와 JSON 포맷으로 변환하는 dataMarshalling() 과정을 자체 수행합니다.

[AI 연동 및 결과 생성]
AI 리포트 분석: Engine은 GeminiAPI를 호출하여 심층 분석을 요청하고, 응답받은 결과를 리포트 객체(buildReportObject)로 조립합니다.

결과 렌더링 및 세션 진입: 최종 리포트 객체를 UI로 반환하며, UI는 이를 차트 및 텍스트 형태로 시각화합니다. 시스템은 분석이 완료된 후 사용자가 결과를 확인하는 ReportSession 상태에 진입합니다.

### 다이어그램 설계 특징
UML 표준 프레임 구현: rect 문법을 사용해 다이어그램 최외각에 배경 박스를 씌우고 좌측 상단에 [Interaction Generate Analysis Result] 노트를 배치하여 표준 규격의 다이어그램 프레임 범위를 명확히 했습니다.

비동기 처리 시각화: activate와 deactivate를 API 호출 구간마다 정교하게 배치함으로써, 각 객체가 실제 연산을 수행하는 구간과 응답을 기다리는 구간(Idle)을 명확히 구분하여 모델링의 정확성을 높였습니다.

상태 추적성: 객체 가명(Alias) 뒤에 상태 모델링 명칭을 병기하여, 해당 프로세스가 전체 시스템 라이프사이클 중 어느 상태에서 동작하는지 직관적으로 파악할 수 있도록 했습니다.
...
본 주식 분석 시스템의 아키텍처 설계, 컴포넌트 간 상호작용 및 알고리즘 구현에 사용된 핵심 기술 용어 정의는 아래와 같다.

| Terms | Description |
| :--- | :--- |
| **Sequence Diagram** | 객체 간의 동적인 상호작용을 시간 순서에 따라 시각화한 UML 다이어그램. 시스템의 파이프라인 흐름을 규명하는 데 필수적임. |
| **State Machine Diagram** | 시스템의 생명 주기 내 상태 전이(State Transition) 과정을 추상화하여 표현한 다이어그램. 이벤트에 따른 UI 및 시스템 상태 변경을 정의함. |
| **API (Application Programming Interface)** | Gemini API 및 Stock API와 같이 외부 서비스와의 인터페이스를 규격화하여 데이터를 상호 교환하기 위한 연결 고리. |
| **Data Marshalling** | 시스템 컴포넌트 간 데이터를 전송하기 위해 직렬화(Serialization)하거나 특정 데이터 포맷(JSON, CSV)으로 변환하는 과정. |
| **Execution Context** | 메서드 호출 시 생성되는 실행 스택(Call Stack)의 범위. 다이어그램 상에서 활성화 박스(Activation Bar)로 표기되어 프로세스의 점유 시간을 나타냄. |
| **Encapsulation** | 객체의 내부 데이터(멤버 변수)와 연산(메소드)을 외부로부터 보호하고, 인터페이스를 통해서만 상호작용하도록 하는 객체지향 설계 원칙. |
| **JLabel / Swing Component** | JAVA GUI 프레임워크인 Swing의 기본 구성 요소로, 시스템의 정보 가독성을 높이는 텍스트 표현 및 레이아웃 제어용 클래스. |
| **TreeMap** | 레드-블랙 트리(Red-Black Tree)를 기반으로 키-값 쌍을 저장하는 컬렉션 프레임워크. 주식 분석 시 정렬된 데이터 조회가 빈번할 때 성능 최적화를 위해 사용함. |
| **Non-Functional Requirement** | 시스템의 성능, 가용성, 유지보수성 등 기능적 구현 이외에 아키텍처가 충족해야 할 제약 조건 및 품질 속성. |

# 7. References

본 주식 분석 시스템의 설계, 개발 및 기술 구현을 위해 참조한 주요 문헌 및 웹 리소스는 아래와 같다.

| 주제 분야 | 사이트명 / 문헌명 | 주요 참고 내용 |
| :--- | :--- | :--- |
| **소프트웨어 공학** | [OMG UML Specification](https://www.omg.org/spec/UML/) | 시퀀스 다이어그램 및 상태 머신 다이어그램 표준 표기법 |
| **분석 엔진** | [Google Gemini API Docs](https://ai.google.dev/docs) | 주식 데이터 분석 및 리포트 생성 알고리즘 연동 가이드 |
| **데이터 연동** | [Stock Market API Documentation](https://www.alphavantage.co/documentation/) | 실시간 시장 시세 조회 및 데이터 마샬링(Marshalling) 처리 |
| **Java 개발** | [Oracle Java Documentation](https://docs.oracle.com/en/java/) | Swing GUI 컴포넌트 설계 및 객체지향 프로그래밍 원칙 |
| **시스템 설계** | [Refactoring.guru](https://refactoring.guru/) | 시스템 아키텍처 패턴 및 유지보수성을 고려한 코드 구조 설계 |
