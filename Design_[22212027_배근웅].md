...
# [설명서] AI 분석 요청 시퀀스 다이어그램 (정밀 활성화 모델링 버전)

본 명세서는 주식 분석 파이프라인 중 생성형 AI(Gemini API) 연동 및 시맨틱 캐싱 레이어의 동적 흐름을 UML 표준 규격에 맞추어 정밀하게 모델링한 문서입니다. 객체의 실제 실행 시간(Execution Specification)을 세분화하여 설계의 정확도를 높였습니다.

## 1. 정밀 활성화 제어 설계 특징 (UML 표준 준수)

* **호출자(Sender)의 비활성화 상태 유지:** `AnalysisManager`는 메시지를 발송한 후 응답을 대기하는 상태이므로, 불필요하게 생명선을 덮고 있던 롱블록 활성화 박스를 제거하고 깔끔한 점선(Lifeline) 상태를 유지하도록 제어했습니다.
* **자체 호출(Self-Call) 중첩 박스(Nested) 구현:** `requestSemanticCache()`, `Apply criteria`, `saveToSemanticCache()` 등 객체가 자기 자신의 메서드를 호출하는 구간에서는 원래 활성화 박스 위에 **작은 사각형이 한 단계 겹쳐서 생성되는 표준 UML 레이아웃**을 `->>+` 및 `deactivate` 문법으로 구현했습니다.
* **외부 컴포넌트 생명주기 제한:** 외부 연동 서버인 `GeminiAPI`는 `AIHandler`가 포스트 요청을 보내고 응답을 받는 정확한 찰나의 순간에만 활성화 박스(`activate`/`deactivate`)가 켜지도록 제한하여 시스템 자원 점유 흐름을 시각적으로 명확히 했습니다.

---

## 2. 세부 시퀀스 실행 흐름

1. **분석 요청 트리거:** `AnalysisManager`가 점선 상태에서 `AIHandler`에게 검증된 프롬프트를 전송하며 흐름이 시작됩니다.
2. **제어권 확보 및 캐시 체크:** 요청을 받은 `AIHandler`에 주 활성화 박스가 켜지며, 즉시 중첩 박스를 생성해 `requestSemanticCache()` 연산을 처리합니다.
3. **조건 분기 처리 (alt / else):**
   * **Cache Hit:** 외부 호출 없이 세션이 즉시 마무리 단계로 이동합니다.
   * **Cache Miss:** 가이드라인 반영을 위한 자체 연산(`Apply criteria`)을 중첩 박스로 수행한 뒤, 외부 `GeminiAPI`를 활성화시켜 원천 데이터를 수신합니다. 이후 `saveToSemanticCache()` 중첩 연산을 거쳐 캐시를 갱신합니다.
4. **제어권 반환 및 소멸:** 모든 연산이 종료되면 `AIHandler`는 결과를 반환함과 동시에 자신의 활성화 박스를 완전히 소멸시키며 시스템을 대기 상태로 전환합니다.
